...
server <- function(input, output, session) {
  ...
  # Export data handler - fixed to properly export all data
  observeEvent(input$export_data, {
    ...
    showModal(
      modalDialog(
        ...
        tags$div(
          ...
          DT::dataTableOutput("export_data_table")
        ),
        footer = tagList(
          modalButton("Close"),
          # Alternative direct download button for all data
          downloadButton("direct_csv_download", "Download All Data as CSV")
        )
      )
    )
    
    # Render the data table with explicit export options
    ...
    # Direct CSV download handler as a fallback
    output$direct_csv_download <- downloadHandler(
      filename = function() {
        index_name <- switch(input$selected_index,
                            ...
                            "inflation" = "Inflation")
        paste0(index_name, "_data_", Sys.Date(), ".csv")
      },
