...
```{ojs}
//| echo: false
// Percent-change and absolute-value indicators differ only in whether the
// series is re-indexed to its reference value, so this is one pass with a flag.
// (The retired Shiny app split this into process_market_data() and
// process_econ_data(); see the history before the OJS rewrite.)
//
