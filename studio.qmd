---
title: "Dev Studio"
subtitle: "Observable JS rebuild — full feature parity with the Shinylive app"
format:
  html:
    theme: morph
    page-layout: full
...
The Observable JS rebuild of the scorecard, at feature parity with the Shinylive app on the
[Scorecard](index.qmd) tab: all eleven indicators, both the percent-change and absolute-value
paths, party filtering, and both exports. Flip between the two tabs to compare.

Data comes from `data/*.json`, written by `get_data.R`. Only the selected indicator is fetched,
so the page loads about 90 KB gzipped rather than the 3.9 MB CSV plus webR and five R packages.

No R runs in the browser. The downloaded PNG is a render of the chart on screen rather than a
separately-composed ggplot — the one real difference from the Shinylive version.
:::

```{=html}
<style>
/* Quarto stamps .card.bg-light onto .panel-sidebar, so these have to out-specify
   it rather than just set the property. The Shinylive app puts everything on a
   white card; match that so the two pages read as the same site. */
.panel-sidebar.card {
  width: 300px;
  flex: 0 0 300px;
...
/* Observable Inputs renders both the group heading and each option as <label>.
   ...
   that holds the controls. Only the heading is bold, matching the Shinylive app. */
...
/* Observable Inputs lays checkbox options out in an auto-fill grid, which gives
   two ragged columns at this width. The Shinylive app uses one column. */
...
/* The indicator blurb the Shinylive app shows above the chart. */
.indicator-note {
  background: #f4f7fb;
...
```{ojs}
//| echo: false
// Inputs.range puts a full-width number box above the track. The Shinylive app
// uses a bare track with tick labels underneath, so this is a hand-rolled view:
